...
import { supabase } from "../supabase"; // uses the client above
...
const CommentForm = memo(({ onSubmit, isSubmitting }) => {
  ...
  const handleSubmit = useCallback(
    ...
    [newComment, userName, imageFile, onSubmit, isSubmitting]
  );
...
const Komentar = () => {
    ...
    const fetchPinnedComment = async () => {
      try {
        ...
        if (error && error.code !== "PGRST116") {
          console.error("Pinned comment fetch error:", error);
          return;
        }

        if (data) setPinnedComment(data);
      } catch (err) {
        console.error("Pinned comment fetch error:", err);
      }
    };
  ...
  /* Fetch regular comments + realtime */
